...
Tweets.location <- read.csv("tweet_Niks.csv",header=TRUE,sep=",")
...
# Correlation Matrix
corr<-Tweets.data %>% select(negativereason_confidence,retweet_count)
...
conf_mat1_plot <- ggplot(conf_mat1$table, aes(Prediction, Reference, fill = Freq , label=Freq)) + 
...
rf_accuracy <- accuracy(rf_predict2,test_set$airline_sentiment)

# Evaluate model performance
conf_mat2 <- confusionMatrix(rf_predict2,test_set$airline_sentiment , mode="everything")
conf_mat2

rf_f1_score <- F1_Score(test_set$airline_sentiment, rf_predict2)
rf_f1_score

conf_mat2_plot <- ggplot(conf_mat2$table, aes(Prediction, Reference, fill = Freq , label=Freq)) + 
geom_tile() + geom_text(color="white") +scale_fill_gradientn(colors=my_colors) + 
theme_minimal() + labs(title = "Confusion Matrix", x = "Predicted", y = "Actual")
conf_mat2_plot

######### EXTRA CODE ##########################
precision_neg1 <- conf_mat$byClass[1,"Precision"]
recall_neg1 <- conf_mat$byClass[1,"Recall"]

precision_neu1 <- conf_mat$byClass[2,"Precision"]
recall_neu1 <- conf_mat$byClass[2,"Recall"]

precision_pos1 <- conf_mat$byClass[3,"Precision"]
recall_pos1<- conf_mat$byClass[3,"Recall"]
...
test_accuracy <- accuracy(knn_predict_test, test_set$airline_sentiment)
print(test_accuracy)

# Evaluate model performance
conf_mat3 <- confusionMatrix(knn_predict_test,test_set$airline_sentiment , mode="everything")
conf_mat3

conf_mat3_plot <- ggplot(conf_mat3$table, aes(Prediction, Reference, fill = Freq , label=Freq)) + 
geom_tile() + geom_text(color="white") +scale_fill_gradientn(colors=my_colors) + 
theme_minimal() + labs(title = "Confusion Matrix", x = "Predicted", y = "Actual")
conf_mat3_plot
...
conf_mat3_plot <- gplot(conf_mat3$table, aes(Prediction, Reference, fill = Freq , label=Freq)) + 
geom_tile() + geom_text(color="white") +scale_fill_gradientn(colors=my_colors) + 
theme_minimal() + labs(title = "Confusion Matrix", x = "Predicted", y = "Actual")
